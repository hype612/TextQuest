...
Player::Player(float x_pos, float y_pos, float angle, float fov, std::wstring& map, int map_width, int map_height) :
	m_x_pos(x_pos), m_y_pos(y_pos), m_angle(angle), m_fov(fov), m_map(map), _map_width(map_width), _map_height(map_height) {}
...
{
	m_x_pos += rval_x;
	if (m_map[(int)m_y_pos * _map_width + (int)m_x_pos] == '#')
		m_x_pos -= rval_x;
